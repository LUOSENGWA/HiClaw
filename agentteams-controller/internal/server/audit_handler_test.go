...
// TestAuditFromAfterToIs400 pins the range error mapping.
func TestAuditFromAfterToIs400(t *testing.T) {
	r := newAuditRig(t)
	rec := r.do(t, "?from=2026-09-15T00:00:00Z&to=2026-09-14T00:00:00Z", auditAdminCaller())
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("from>to: want 400, got %d: %s", rec.Code, rec.Body.String())
	}
}
